 ...
 * link. This panel holds both — adding or removing an invitation takes effect
 * on the next page load, with no redeploy, which is the whole point of it
 * living in the app rather than in a dashboard.
 *
 * The password form shows for everyone who can see this panel, including
 * people who signed in with Google. It used to be hidden from them, which made
 * sense only while Google sign-in *replaced* the password; now that both doors
 * are open at once the password is live either way, and hiding it meant the
 * admins were the one group who could not change it.
 *
...
export function UsersPanel() {
  ...
  // An address typed but not invited yet.
  useReportDirty("users", Boolean(newEmail.trim()));
