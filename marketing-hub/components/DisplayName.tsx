...
/** Who is editing, as initials in a circle with the name beside them. */
export function DisplayNameBadge() {
  ...
  if (name === null) return <span className="who" aria-hidden />;
  ...
  // Under Google sign-in the name is changed from Settings, so this should
  // not look like a button that does nothing when pressed.
  if (verified) {
    return (
  ...
  return (
    <button
      ...
      title={name ? "Change the name your edits are stamped with" : "Set your name"}
    >
      <span className="who__avatar" aria-hidden>
        {initials || "?"}
      </span>
      <span className="who__name">{name || "Set your name"}</span>
    </button>
  );
}
