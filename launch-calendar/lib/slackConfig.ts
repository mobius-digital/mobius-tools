...
export async function channelMap(): Promise<SlackChannelMap> {
  const [{ results }, keys] = await Promise.all([
    getDb()
      .prepare(`SELECT channel_key, slack_id, slack_name FROM slack_channel_map`)
...
export async function mapChannel(
  ...
  if (!slackId || typeof slackId !== "string") {
    await getDb()
      .prepare(`DELETE FROM slack_channel_map WHERE channel_key = ?`)
      .bind(channelKey)
      .run();
    return { ok: true };
  }
  ...
  await getDb()
    .prepare(
      `INSERT INTO slack_channel_map (channel_key, slack_id, slack_name, updated_at)
       VALUES (?, ?, ?, ?)
    ...
    .bind(channelKey, slackId, String(slackName ?? slackId), new Date().toISOString())
    .run();
