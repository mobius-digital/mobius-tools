  ...
  await getDb()
    .prepare(
      `INSERT INTO slack_outbox
         (id, event_id, event_name, kind, lines, channel_keys, actor, created_at, due_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .bind(
      crypto.randomUUID(),
...
export async function flushOutbox(
  ...
  const { results } = await db
    .prepare(
      `SELECT id, event_id, event_name, kind, lines, channel_keys, actor, attempts, sent_to
       FROM slack_outbox
       WHERE sent_at IS NULL AND (due_at <= ? OR ?)
       ORDER BY created_at ASC
       LIMIT ?`,
    )
    .bind(stamp, options.force ? 1 : 0, FLUSH_LIMIT)
    .all<OutboxRow>();
  ...
  if (!settings.enabled) {
    await db
      .prepare(
        `UPDATE slack_outbox SET sent_at = ? WHERE sent_at IS NULL AND (due_at <= ? OR ?)`,
      )
      .bind(stamp, stamp, options.force ? 1 : 0)
      .run();
    return { posted: 0 };
  }
  ...
  const row = await getDb()
    .prepare(
      `SELECT 1 AS ok FROM slack_reminders_sent
       WHERE event_id = ? AND kind = ? AND launch_date = ?`,
    )
    .bind(eventId, kind, launchDate)
    .first();
  return Boolean(row);
}
...
async function markReminded(
  ...
  await getDb()
    .prepare(
      `INSERT INTO slack_reminders_sent (event_id, kind, launch_date, sent_at)
       VALUES (?, ?, ?, ?) ON CONFLICT DO NOTHING`,
    )
    .bind(eventId, kind, launchDate, new Date().toISOString())
    .run();
}
...
export async function pendingCount(): Promise<{ pending: number; nextDue: string | null }> {
  const row = await getDb()
    .prepare(`SELECT COUNT(*) AS n, MIN(due_at) AS due FROM slack_outbox WHERE sent_at IS NULL`)
...
export async function pruneSlackHistory(now: Date = new Date()): Promise<void> {
  ...
  await db
    .prepare(`DELETE FROM slack_outbox WHERE sent_at IS NOT NULL AND sent_at < ?`)
    .bind(cutoff)
    .run();

  await db
    .prepare(`DELETE FROM slack_reminders_sent WHERE launch_date < ?`)
    .bind(addDays(todayIso(now), -60))
    .run();
}
