...
ck('T4 first run creates 1 (Client B only; expense rows are never pre-created; paid Client A skipped)', rc1.get('created') == 1, rc1)
ck('T4 second run creates 0', rc2.get('created') == 0, rc2)
s, txns = api('/api/transactions?month=2026-04')
exp_rows = [t for t in txns['transactions'] if t['expected'] == 1]
ck('T4 exactly 1 expected row, and it is revenue', len(exp_rows) == 1 and exp_rows[0]['type'] == 'in', [t['vendor'] for t in exp_rows])
...
s, c = api('/api/close', 'POST', {'month':'2026-04'})
ck('T5 close refused: expected unconfirmed', s == 400 and c.get('expected') == 1, (s, c))
