...
function timelineItems(s) {
  ...
  for (const sl of s.slots) {
    ...
    items.push({ kind: 'slot', group: sl.collectionName || 'New designs', label: sl.name, sub: `${sl.lineName || ''} · ${SLOT_STATUS[sl.status]?.[0] || sl.status}`, open: `openSlot('${sl.id}')`,
      phases: [[d.briefDue, d.techPackDue, 'design', '#14608C'], [d.techPackDue, d.sampleDue, 'sample', '#62BDEA'], [d.sampleDue, d.orderBy, '', '#DCE7EE'], [d.orderBy, d.productionEnd, f ? `production ${f.production_days} d` : 'production', '#C9962A'], [d.productionEnd, d.lands, 'ship', '#7A8794'], [d.lands, d.onSite, '', '#C3CED8']],
